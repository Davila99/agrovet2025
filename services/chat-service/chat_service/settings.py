"""
Django settings for chat-service microservice.
"""
...
REST_FRAMEWORK = {
    'DEFAULT_AUTHENTICATION_CLASSES': (
        'chat.authentication.AuthServiceTokenAuthentication',
...
# Redis Configuration (for Channel Layers and presence)
...
CHANNEL_LAYERS = {
    "default": {
        ...
        "CONFIG": {
            "hosts": [(REDIS_URL.replace('redis://', '').split('/')[0].split(':')[0],
                      int(REDIS_URL.replace('redis://', '').split('/')[0].split(':')[1]) if ':' in REDIS_URL.replace('redis://', '').split('/')[0] else 6379)],
        },
    },
}
